...
\begin{proof}
    ...
    V $i$-tem koraku operacija \textit{rescanning} obišče $n_i$ vozlišč. Pri tem se opazi, da v naslednjem koraku ta vozlišča ne bodo obiskana. Iz tega sledi:  $|\beta_{i+1}\gamma_{i+1}|\le|\beta_i\gamma_i|-n_i$, pričemer sta $|\beta_0\gamma_0|=n$ in $|\beta_n\gamma_n|=0$. Torej velja tudi:
...
Ukkonenov algoritem deluje na podoben način kot prej predstavljena naivna metoda, saj dodaja v drevo črko po črko. Torej algoritem v $i$-tem koraku izgradi priponsko drevo, ki je lahko implicitno priponsko drevo in predstavlja besedilo $T[1:i]$. Primer drevesa zgrajenega z Ukkonenovim algoritmom je prikazana na Sliki \ref{fig:Ukkonen}. Algoritem doda, v drevo, v $i$-tem koraku črko $x$. Naj bo $\alpha$ niz, ki z $x$ tvori niz $T[1:i]=\alpha x$, $\alpha$ je lahko prazen niz. Znak $x$ je lahko dodan v drevo na tri načine:
...
Razčlemba in dodajanje novih vozlišč se začne v aktivni točki (angl. \textit{active point}) in se konča v končni točki (angl. \textit{end point}). Na Sliki \ref{fig:Ukkonen} je z rdečo označeno vozlišče, ki se nahaja pred končno točko, ter z zeleno je označeno vozlišče, ki pa se nahaja pred začetno aktivno točko. Aktivna točka je točka na povezavi, v kateri se bo zgodila razčlemba povezave. Končna točka pa je zadnja aktivna točka v trenutnem koraku dodajanja. Premik med aktivno točko in končno točko poteka po priponskih povezavah. Pri tem pa algoritem v vsakem koraku poti izračuna ali je že prišel v končne točke. Za to je potrebno hraniti zgolj trenutno točko ter zastavico, ki hrani vrednost ali je ta točka tudi končna točka. Pri tem velja tudi, da končna točka v koraku $i-1$ postane aktivna točka v koraku $i$. 

Aktivna točka je predstavljena kot referenčni par $(s,w)$, pri čemer je $s$ vozlišče pred aktivno točko in $w$ je niz iz vozlišča $s$ do aktivne točke. Za lažje shranjevanje niza $w$ je ta shranjen kot par indeksov $(k,p)$, pri čemer je $w=T[k,p]$. Ker je lahko aktivna točka predstavljena z različnimi referenčnimi pari $(s,(k,p))$, se izbere kanonično obliko referečnega para. Kanonična oblika točke je referenčni par $(s,(k,p))$, za katerega velja, da $s$ je najgobje vozlišče pred točko in posledično niz $w=T[k,p]$ je najkrajši. Na Sliki \ref{fig:Ukkonen} v četrtem koraku izgradnje (prvo drevo v spodnji vrstici) ima začetno aktivno točko  predstavljeno kot $(\textit{koren},(1,3))$, v naslednjem koraku pa je začena aktivna točka v $(\textit{koren},(1,4))$, ki se bo razcepila in bo postala novo vozlišče. Ta ista točka po koncu izgradnje je lahko še vedno predstavljena z referenčnim parom $(\textit{koren},(1,4))$, ki pa ni v kanonični obliki. Kanonična oblika točke je $(a,(4,4))$, pri čemer je vozlišče $a$ otrok \textit{koren}-a, na katerega kaže povezava z nizom »KO«.
...
Algoritem \ref{alg:ukkonen} uporabi dve pomožni funkciji. Prva uporabljena pomožna funkcija je \texttt{kanoničnaOblika}, ki pretvori trenutno aktivno točko priponskega drevesa, v kanonično obliko. Funkcija prejme kot vhod vozlišče $s$ ter niz predstavljen kot položaj začetne in končne točke v nizu $k$ in $p$. Funkcija se sprehodi po drevesu dokler ne doseže najnižjega vozlišča pred aktivno točko. S tem korakom je omogočena lažja uporaba funkcije \texttt{razdeliTestiraj}.
