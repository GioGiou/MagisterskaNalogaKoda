...
V nadaljevanju tega dela poglavja bo predstavljena preprosta različica kopaktenga priponskega polja. Ta različica je bila izbrana zaradi enostavne implementacije operacije ter preproste vizualizacije strukture. V tej implementacije so vse štiri operacije implementirane z uporabo $\Psi$ funkcije ter z vzorčenjem priponskega polja $SA$ in inverznega priponskega polja $SA^{-1}$. Na ta način ni potrebno hraniti besedila $T$ in celotnega priponskega polja $SA$.

Funkcija $\Psi$ je predstavljena z istoimenskim poljem, katerega $i$-ta celica je $\Psi[i]=\Psi(i)=SA^{-1}[SA[i]+1]$. Ideja kompaktnega zapisa polja $\Psi$ temelji na dejstvu, da za $j=SA[i]$ in $j'=SA[i+1]$ ter $T[j]=T[j']$ sledi $T[j+1,n]<T[j'+1,n]$ (leksikografsko), torej tudi $\Psi(i)<\Psi(i+1)$. To dejstvo se lahko zapiše kot bitno polje $D$, pri čemer $D[i]=1$, ko je $i=1$ ali $T[SA[i]]\ne T[SA[i+1]]$, ter polje znakov $S$ urejenih v leksikografskem vrstnem redu, tako da $T[SA[i]]=S[rang_1(D,i)]$.
